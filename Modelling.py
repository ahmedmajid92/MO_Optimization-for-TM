...
###########################################################
# Clustering Step using K-Means Algorithm
###########################################################

'''cluster = KMeans(n_clusters=80, init='k-means++', random_state=0, n_init=20).fit(embeddings)
...
###########################################################
# Clustering Step using BisectingKMeans Algorithm
###########################################################
'''
cluster = BisectingKMeans(n_clusters=12,
                          init='k-means++',
                          n_init=5,
                          random_state=0,
                          #algorithm='elkan',
                          max_iter=300,).fit(embeddings)

print("No. of Clusters is : ",cluster.labels_.max() + 1)
'''
###########################################################
# Clustering Step using Birch Algorithm
###########################################################

'''cluster = Birch(threshold=0.5,
                branching_factor=400,
                n_clusters=8).fit(embeddings)
